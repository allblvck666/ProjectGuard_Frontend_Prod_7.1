...
function App() {
  ...
  const submitEdit = async () => {
    ...
    try {
      await api.put(`${API}/api/protections/${editModal.id}`, payload);
  ...
  const loadHistory = async () => {
    const r = await api.get(`${API}/api/history`);
    setHistory(r.data || []);
  };
  ...
  useEffect(() => {
    ...
    api.get(`${API}/api/managers`).then((r) => {
  ...
  const submit = async () => {
    ...
    try {
      await api.post(`${API}/api/protections`, payload);
      setForm({
        manager: "",
        client: "",
        partner: "",
      ...
      if (typeof detail === "string") {
        ...
        if (reason && reason.trim()) {
          try {
            await api.post(`${API}/api/protections/pending`, {
              ...payload,
              comment: reason.trim(),
            });
  ...
  const extendAction = async (id, days = 10) => {
    try {
      await api.post(`${API}/api/protections/${id}/extend?days=${days}`);
      await load();
    } catch (err) {
      ...
      if (err.response?.status === 403 && (det?.needs_admin || det?.msg)) {
        ...
        if (reason && reason.trim()) {
          await api.post(`${API}/api/protections/${id}/request-extend`, {
            days,
            reason: reason.trim(),
          });
  ...
  const doClose = async () => {
    try {
      await api.post(`${API}/api/protections/${closeModal.id}/close`, {
  ...
  const doSuccess = async () => {
    try {
      await api.post(`${API}/api/protections/${successModal.id}/success`, {
  ...
  const doDelete = async () => {
    try {
      await api.delete(`${API}/api/protections/${deleteModal.id}`, {
